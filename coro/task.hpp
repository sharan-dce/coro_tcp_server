// This has been taken from https://github.com/lewissbaker/cppcoro
...
public:
    ...
    std::optional<std::exception_ptr> get_safe_exception() const {
        return std::holds_alternative<std::exception_ptr>(m_storage) ?
            std::get<std::exception_ptr>(m_storage) :
            std::nullopt;
    }
...
public:
    ...
    auto done() const noexcept -> bool { return m_coroutine == nullptr || m_coroutine.done(); }
